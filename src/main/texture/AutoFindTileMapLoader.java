...
public class AutoFindTileMapLoader implements TileMapLoader {
    ...
    public AutoFindTileMapLoader( String file, boolean shouldRemoveBackground ) {
        loader = new BasicTextureLoader( file, shouldRemoveBackground, true );
    }
    ...
    private static TwoTuple<Point, Point> extractSprite( BufferedImage source, Color background, boolean[][] searched, Point loc )
    {
        ...
        while( !search.isEmpty() ) {
            ...
            if( cur.x <= SEARCH_SIZE - 1 ) mix = 0; else mix = cur.x - SEARCH_SIZE;
            if( cur.x >= source.getWidth() - SEARCH_SIZE ) max = source.getWidth() - SEARCH_SIZE; else max = cur.x + SEARCH_SIZE;
            if( cur.y <= SEARCH_SIZE ) miy = 0; else miy = cur.y - SEARCH_SIZE;
            if( cur.y >= source.getHeight() - SEARCH_SIZE ) may = source.getHeight() - SEARCH_SIZE; else may = cur.y + SEARCH_SIZE;
